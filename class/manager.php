 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_custom_progress_manager {
    ...
    const USERPREF_NOTIFY = 'block_custom_progress_notify_level_up_';

    /** User preference key storing if they dismissed the like notice */
    const USERPREF_NOTICES = 'block_custom_progress_notices';
    ...
    /** @var block_custom_progress_filter_manager Cache of the manager. */
    ...
    protected function __construct($courseid) {
        ...
        if ($CFG->block_custom_progress_context == CONTEXT_SYSTEM && $courseid != SITEID) {
    ...
    protected function can_capture_event(\core\event\base $event) {
        ...
        if (!isset($SESSION->block_custom_progress_cheatguard)) {
            $SESSION->block_custom_progress_cheatguard = array();
        }

        // Actions per time.
        if (count($SESSION->block_custom_progress_cheatguard) > $maxactions) {
            $actions = array_reverse($SESSION->block_custom_progress_cheatguard, true);
            $count = 0;
        ...
        if (isset($SESSION->block_custom_progress_cheatguard[$actionkey])) {
            if ($SESSION->block_custom_progress_cheatguard[$actionkey] > $now - $actiontime) {
        ...
        unset($SESSION->block_custom_progress_cheatguard[$actionkey]);

        // Log the time at which this event happened.
        $SESSION->block_custom_progress_cheatguard[$actionkey] = time();

        // Limit the array of events to $maxcount, we do not want to flood the session for no reason.
        $SESSION->block_custom_progress_cheatguard = array_slice($SESSION->block_custom_progress_cheatguard, -$maxcount, null, true);
        return true;
    }
    ...
    public function capture_event(\core\event\base $event) {
        ...
        if ($CFG->block_custom_progress_context != CONTEXT_SYSTEM && $event->courseid != $this->courseid) {
        ...
        $userid = $event->userid;
        $points = $this->get_custom_progress_from_event($event);

        // No need to go through the following if the user did not gain XP.
        if ($points > 0) {
            if ($DB->count_records('block_custom_progress', array('courseid' => $this->courseid, 'userid' => $userid)) > 0) {
                $DB->execute('UPDATE {block_custom_progress} SET xp = xp + :xp WHERE courseid = :courseid AND userid = :userid',
                ...
                $record->xp = $points;
                $DB->insert_record('block_custom_progress', $record);
            }
     ...
     * @return block_custom_progress_manager The instance of the manager.
     */
    public static function get($courseid, $forcereload = false) {
        ...
        if ($CFG->block_custom_progress_context == CONTEXT_SYSTEM) {
            $courseid = SITEID;
        }

        $courseid = intval($courseid);
        if ($forcereload || !isset(self::$instances[$courseid])) {
            self::$instances[$courseid] = new block_custom_progress_manager($courseid);
        }
    ...
    public function get_config($name = null) {
        ...
        if (empty($this->config)) {
            $record = $DB->get_record('block_custom_progress_config', array('courseid' => $this->courseid));
            if (!$record) {
     ...
     * courses ($CFG->block_custom_progress_context == CONTEXT_COURSE), then the context of the manager
     * should be the course context. However, when block_custom_progress_progress is set to CONTEXT_SYSTEM
     ...
     *
     * When the block is set to act on the whole site ($CFG->block_custom_progress_context == CONTEXT_SYSTEM),
     ...
     *
     * @return block_custom_progress_filter_manager
     */
    public function get_filter_manager() {
        if (!$this->filtermanager) {
            $this->filtermanager = new block_custom_progress_filter_manager($this);
        }
     ...
     */
    public function get_level_from_custom_progress($xp) {
    ...
    public function get_levels_data() {
        ...
        return array(
            'usealgo' => 1,
            'base' => block_custom_progress_manager::DEFAULT_BASE,
            'coef' => block_custom_progress_manager::DEFAULT_COEF,
     ...
     * @return block_custom_progress_progress The progress renderable.
     */
    public function get_progress_for_user($userid, stdClass $record = null) {
        global $DB;

        if (!$record) {
            $record = $DB->get_record('block_custom_progress', array('courseid' => $this->courseid, 'userid' => $userid));
        }
        ...
        $params['levelxp'] = $this->get_custom_progress_for_level($record->level);
        $params['nextlevelxp'] = $this->get_custom_progress_for_level($record->level + 1);
        $progress = new block_custom_progress_progress($params);
     ...
     */
    public function get_custom_progress_from_event(\core\event\base $event) {
     ...
     */
    public function get_custom_progress_for_level($level) {
        $levels = $this->get_levels();
    ...
    protected function log_event($eventname, $userid, $xp) {
        ...
        if ($this->get_config('enablelog')) {
            ...
            $record->time = time();
            try {
                $DB->insert_record('block_custom_progress_log', $record);
    ...
    public function purge_log() {
        ...
        if (!$keeplogs) {
            ...
            $DB->delete_records_select('block_custom_progress_log', 'time < :time', array(
    ...
    public function recalculate_levels() {
        global $DB;
        $users = $DB->get_recordset('block_custom_progress', array('courseid' => $this->courseid), '', 'userid, lvl, xp');
    ...
    public function reset_data($groupid = 0) {
        ...
        // Delete XP records.
        $sql = "DELETE FROM {block_custom_progress} WHERE courseid = :courseid";
        ...
        // Delete logs.
        $sql = "DELETE FROM {block_custom_progress_log} WHERE courseid = :courseid";
        $params = array('courseid' => $this->courseid);
        if ($groupid) {
            $sql .= " AND userid IN
                  (SELECT gm.userid
     ...
     * @return void
     */
    public function reset_user_custom_progress($userid, $xp = 0) {
        global $DB;

        if ($record = $DB->get_record('block_custom_progress', array('courseid' => $this->courseid, 'userid' => $userid))) {
            $record->xp = $xp;
            $DB->update_record('block_custom_progress', $record);
        } else {
            $record = new stdClass();
            $record->courseid = $this->courseid;
            $record->userid = $userid;
    ...
    public function update_config($data) {
        ...
        if (empty($config->id)) {
            $config->id = $DB->insert_record('block_custom_progress_config', $config);
        } else {
            $DB->update_record('block_custom_progress_config', $config);
        }
        $this->config = $config;
    }
    ...
    public function update_user_level($userid, $xp = null, $lvl = null) {
        ...
        if ($xp === null || $lvl === null) {
            $record = $DB->get_record('block_custom_progress', array('courseid' => $this->courseid, 'userid' => $userid), 'xp,lvl');
            if (!$record) {
                return;
            }
            $xp = $record->xp;
        ...
        $level = $this->get_level_from_custom_progress($xp);
        if ($level != $lvl) {
            // Level up!
            $DB->set_field('block_custom_progress', 'lvl', $level, array('courseid' => $this->courseid, 'userid' => $userid));
            if ($this->triggereevents) {
                ...
                $event = \block_custom_progress\event\user_leveledup::create($params);
                $event->trigger();
            }
        }
